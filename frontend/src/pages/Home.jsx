...
const HeroArenaWidget = ({ matchData, upcomingMatches }) => {
    ...
    return (
        <motion.div 
            ...
            className={`relative w-full overflow-hidden rounded-[40px] border border-white/10 shadow-3xl transition-all duration-700 ${
                isBoundary ? "shadow-[0_0_80px_rgba(251,191,36,0.3)] border-amber-500/50" : ""
            }`}
        >
            {/* Dynamic Backgrounds */}
            <div className={`absolute inset-0 z-0 transition-opacity duration-1000 ${
                isLive ? "bg-gradient-to-br from-red-600 via-orange-600 to-black" : "bg-gradient-to-br from-indigo-900 via-slate-900 to-black"
            }`} />
        ...
        <div className="bg-[#0c0c14] border border-white/5 rounded-[40px] p-8 flex flex-col justify-between h-full relative overflow-hidden group">
            ...
            <button 
                ...
                className={`w-full h-14 rounded-2xl font-black uppercase text-xs tracking-widest transition-all ${
                    isActive ? "bg-white/10 text-white border border-white/10" : "bg-indigo-600 text-white shadow-lg shadow-indigo-500/20"
                }`}
            >
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`relative z-10 text-[9px] font-black uppercase tracking-[0.2em] transition-all duration-300 ${
                                    isActive ? "text-white opacity-100" : "text-white/20 opacity-0 scale-90"
                                }`}>
                                    {tab.label}
                                </span>
                            </button>
