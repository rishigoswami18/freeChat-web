...
// Extracted outside the component to prevent memory recreation on every render cycle
...
// === REUSABLE DISGUISE THEMES ===
// Currently implemented: "LexiLearn Dictionary"
// Extracted out into a strict boundary. Designed to easily scale into a 
// Strategy Pattern later (e.g., ThemeCalculator, ThemeNotes) without rewriting core logic.
const LexiLearnTheme = memo(({ onExit }) => {
    // Localized state: typing in the dict search won't trigger global App re-renders
                    ...
                    <div className="pt-6 border-t">
                        ...
                        <div className="space-y-2">
                            <div className="p-3 bg-base-200 rounded-xl space-y-2">
                                ...
                                <progress className="progress progress-primary w-full h-1.5 border-none" value="75" max="100"></progress>
                            </div>
                            <div className="p-3 bg-base-200 rounded-xl space-y-2">
                                <div className="flex justify-between items-center text-xs">
                                ...
                                <progress className="progress progress-secondary w-full h-1.5 border-none" value="40" max="100"></progress>
                            </div>
                        </div>
                    </div>
                </aside>
                    ...
                    <div className="space-y-8">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {DICTIONARY_ENTRIES.map((entry, i) => (
                                    <div key={`dict-${i}`} className="p-4 border rounded-2xl hover:bg-base-200 cursor-pointer transition-colors group">
...
LexiLearnTheme.displayName = "LexiLearnTheme";

// === MAIN OVERLAY COMPONENT ===
const StealthOverlay = memo(({ children }) => {
    const { isStealthMode, toggleStealthMode, panicShortcut } = useStealthStore();

    // Stable exit handler ensuring child themes don't disconnect hooks unnecesarily
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e) => {
            // IGNORE panic shortcut if the user is typing naturally inside an input field or textarea
    ...
    useEffect(() => {
        ...
        const handleTouchStart = (e) => {
            // Prevent accidental triggering while multi-finger scrolling
            ...
            if (tapCount === 3) {
                ...
                // Garbage collect the counter if they don't complete the triple tap fast enough
        ...
        // Passive listener ensures touch tracking never halts the main browser scrolling thread
    ...
    return (
        <>
            {/* 
            The underlying child DOM intentionally persists allowing native WebRTC video streams
            and React query websockets to safely continue polling in the background uninterrupted.
            */}
            {children}

            {/* Conditionally mount the selected Stealth Theme exclusively rendering when ON */}
            {isStealthMode && <LexiLearnTheme onExit={handleExit} />}
        </>
    );
});

StealthOverlay.displayName = "StealthOverlay";
